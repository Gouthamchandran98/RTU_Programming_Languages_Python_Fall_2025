...
# Compute totals using map() and zip()
pairs = list(zip(prices, quantities))
totals = list(map(lambda pq: pq[0] * pq[1], pairs))
...
print("Totals:", [f"{t:.2f}" for t in totals])
print("Totals > 30:", [f"{t:.2f}" for t in high_totals])
print("Totals (comprehension):", [f"{t:.2f}" for t in totals_comp])
print("Totals > 30 (comprehension):", [f"{t:.2f}" for t in high_totals_comp])
